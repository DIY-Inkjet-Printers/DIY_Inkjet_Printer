...
import numpy
hight = (300.0)
tool00range = [(x,y) for x in numpy.arange(0.0, 8.0, 1.0) for y in numpy.arange(hight)]
...
dbyte0 = {k: (v >> 8) & 0xFF for k, v in dsum.items()}
dbyte1 =  {k: v & 0xFF for k, v in dsum.items()}
...
raster = [(x,y) for x in nodoublesx for y in numpy.arange(ymin, ymax, 0.5)]
...
sraster = sorted(rasterall , key=lambda k: [k[0][0], k[0][1]])
...
dstr0 = dstr.replace("[([", startcode + "\n")
dstr1 = dstr0.replace("])])]", "\n" + endcode)
dstr2 = dstr1.replace("),),", "),")
dstr3 = dstr2.replace("']),", "',")
dstr4 = dstr3.replace("),", " " + speed + "\n")
dstr5 = dstr4.replace("/", "\n")
dstr6 = dstr5.replace("',", "\n")
dstr7 = dstr6.replace("((", "X")
dstr8 = dstr7.replace(", ", " Y")
dstr9 = dstr8.replace("'", "")
dstr10 = dstr9.replace(" X", "X")
dstr11 = dstr10.replace(" [", "")
dstr12 = dstr11.replace(")] " + speed + "\n ([", "")
dstr13 = dstr12.replace(speed + "\n)] Y[", speed + "\n")
dstr14 = dstr13.replace("M260 S1])] Y[", "M260 S1\n")
dstr15 = dstr14.replace(" M260 A9", "M260 A9")
dstr16 = dstr15.replace("X", "G01 X")
dstr17 = dstr16.replace("M260 S1", "\nM260 S1")
dstr18 = dstr17.replace("M260 A9", "M260 A9\n")
#write to file
